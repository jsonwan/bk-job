...
# 另需基础镜像中带有 python3：同步前要渲染资源文件中的占位符（bin/render_placeholders.py），
# 还要查询 bkai-init 需要的调用用户名（bin/resolve_bkai_username.py），而镜像中没有 curl
FROM hub.bktencent.com/blueking/bk-aidev-init:0.1.0rc8
